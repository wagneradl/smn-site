...
async function analyzeLogoLayout() {
  ...
  try {
    ...
    const clientsSection = await page.$(
      '.bg-gradient-to-br.from-primary-800.to-primary-900',
    )
    if (!clientsSection) {
    ...
    console.log('\n📋 Resumo do layout:')
    console.log(
      `  Grid: ${layoutData.grid.boundingBox.width}x${layoutData.grid.boundingBox.height}`,
    )
    ...
    layoutData.logos.forEach((logo) => {
      console.log(
        `    ${logo.client}: ${logo.boundingBox.image.width}x${logo.boundingBox.image.height} (${logo.classes.wide ? 'wide' : logo.classes.tall ? 'tall' : logo.classes.emblem ? 'emblem' : 'standard'})`,
      )
    })
