...
function analyzeLayout() {
  ...
  const standard = logos.filter(
    (logo) => logo.aspect >= 1.2 && logo.aspect <= 3,
  )
...
console.log(
  '  Wide (>3):',
  layoutAnalysis.classification.wide
    .map((l) => `${l.client} (${l.aspect})`)
    .join(', '),
)
console.log(
  '  Standard (1.2-3):',
  layoutAnalysis.classification.standard
    .map((l) => `${l.client} (${l.aspect})`)
    .join(', '),
)
console.log(
  '  Tall (<1.2):',
  layoutAnalysis.classification.tall
    .map((l) => `${l.client} (${l.aspect})`)
    .join(', '),
)
