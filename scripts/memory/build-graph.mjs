...
function inv(domain, desc, ok, evidence, hint="") {
  graph.invariants.push({
    ...
    domain, desc, status: ok ? "pass" : "fail", evidence, hint
  });
}
...
(() => {
  ...
  inv("MDX", "remark→rehype", order[0]==="remark" && order[1]==="rehype", R.mdx, "Ajuste next.config.mjs");
  inv("MDX", "tema css-variables", !!hasTheme, R.mdx, "createCssVariablesTheme({ name: 'css-variables' })");
  inv("MDX", "@shikijs/rehype presente", !!hasRehype, R.mdx, "adicione plugin de rehype do Shiki");
})();
...
(() => {
  ...
  inv("NAV", "GCPro externo seguro", n?.gcproExternal === true, R.nav, "target=\"_blank\" rel=\"noopener noreferrer\"");
  inv("NAV", "prefetch desativado (GCPro)", n?.gcproPrefetch === true, R.nav, "prefetch={false}");
  inv("A11Y", "foco visível", n?.hasVisibleFocus === true, R.nav, "outline/focus-visible no CSS");
})();
...
(() => {
  ...
  const okBuckets = typeof b.wide==="number" && typeof b.standard==="number" && typeof b.emblem==="number";
  inv("LOGOS", "buckets presentes", okBuckets, R.logos, "reexecute scanner de logos");
  inv("LOGOS", "mínimo 1 por bucket", okBuckets && b.wide>0 && b.standard>0 && b.emblem>0, R.logos,
...
(() => {
  ...
  inv("IMG", "todos next/image com sizes", offenders.length===0, R.img, "corrija sizes por layout");
})();
...
(() => {
  ...
  for (const r of routes) {
    ...
    inv("SEO", `meta em ${r}`, !!row && !!row.title && !!row.description, R.meta, "title/description/og/twitter");
  }
  const home = ld.find(x => x.route === "/");
  const joined = (home?.blocks || []).join("|");
  inv("SEO", "JSON-LD Organization na home", joined.includes('"@type":"Organization"'), R.ld);
  inv("SEO", "JSON-LD WebSite na home", joined.includes('"@type":"WebSite"'), R.ld);

  const robots = http.find((x)=>x.route==="/robots.txt" && x.status===200);
  const sitemap = http.find((x)=>x.route==="/sitemap.xml" && x.status===200);
...
(() => {
  ...
  addNode("lcp", { present: !!lcp, startTime: lcp?.lcp?.startTime ?? lcp?.startTime ?? null });

  inv("PERF", "budgets ok", budgets?.ok === true, R.budgets);
  const max = budgets?.budgets?.firstLoadJS_kb ?? 170;
  inv("PERF", `First Load JS ≤ ${max}kB`, (budgets?.firstLoad ?? 9999) <= max, R.budgets);

  inv("PERF", "LCP registrado", !!(lcp?.lcp || lcp?.startTime), R.lcp, "rode qa:seo (harness efêmero)");
  ...
  for (const h of REQ) {
    inv("SEC", `header ${h}`, cfg.includes(h), R.nextConfig, "configure headers em next.config.mjs");
  }
})();
