...
const importsOk =
  /^@import 'tailwindcss';\s*@import '.\/base.css';\s*@import '.\/typography.css' layer\(components\);/m.test(
    tw,
  )
const hasTheme = /@theme\s*{[\s\S]*}/m.test(tw)

const baseHasLinks = /@layer\s+base\s*{[\s\S]*a\s*{[\s\S]*}/m.test(base)
const baseAccentsOnHover = /a:hover\s*{\s*color:\s*var\(--color-accent-/.test(
  base,
)
