...
try {
  ...
  const jsonFiles = readdirSync(reportsDir).filter(file => file.endsWith('.json'));
...
`;
  if (reports.build) {
    ...
    const firstLoadJS = firstLoadMatch ? `${firstLoadMatch[1]} ${firstLoadMatch[2]}` : 'N/A';
...
`;
  ...
  if (reports.quality?.summary?.criticalIssues > 0) {
    recommendations.push('[CRITICAL] Corrigir erros de TypeScript antes do deploy');
  }

  if (reports.audit?.npmAudit?.summary?.critical > 0) {
    recommendations.push('[CRITICAL] Atualizar dependências com vulnerabilidades críticas');
  }

  if (reports.build?.hasMissingOpeningBrace) {
    recommendations.push('[CRITICAL] Corrigir erro "Missing opening {" no build');
  }

  // Verificar issues altos
  if (reports.audit?.npmAudit?.summary?.high > 0) {
    recommendations.push('[HIGH] Atualizar dependências com vulnerabilidades altas');
  }
  ...
  if (!reports.nextMdxShiki?.shikiConfig?.hasShikijsRehype) {
    recommendations.push('[HIGH] Configurar @shikijs/rehype para syntax highlighting');
  }
  ...
  if (recommendations.length === 0) {
    recommendations.push('[INFO] Nenhum problema crítico encontrado - template em boa condição');
  }
