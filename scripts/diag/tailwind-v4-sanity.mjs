...
try {
  ...
  try {
    ...
    tailwind.postcss.hasTailwindPostcss = postcssContent.includes('@tailwindcss/postcss');
    tailwind.postcss.hasTailwindImport = postcssContent.includes("@import 'tailwindcss'");
  } catch (e) {
  ...
  try {
    ...
    cssFiles.forEach(file => {
      ...
      const nonAsciiChars = content.match(/[\uFEFF\u00A0\u200B\u200C\u200D\u2060]/g);
      if (nonAsciiChars) {
      ...
      if (layerMatches) {
        const layers = layerMatches.map(match => match.replace('@layer', '').trim());
        const expectedOrder = ['base', 'components', 'utilities'];
        const actualOrder = layers.filter(layer => expectedOrder.includes(layer));
      ...
      const hasColorTokens = content.includes('--color-') || content.includes('--smn-');
      if (!hasColorTokens) {
      ...
      const hasLinkGuard = content.includes('[data-button]') || 
