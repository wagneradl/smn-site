...
try {
  ...
  if (navContent) {
    ...
    const navItems = [
      'Sobre nós', 'Sobre nos', 'About',
      'Soluções', 'Solucoes', 'Solutions',
      'Cases', 'Work', 'Portfolio',
      'Carreiras', 'Careers',
      'Blog',
      'GCPro', 'GC Pro'
    ];
    ...
    navGrid.navigation.gcproExternal = navContent.includes('target="_blank"') && 
                                     (navContent.includes('GCPro') || navContent.includes('GC Pro'));

    // Verificar prefetch={false}
    navGrid.navigation.gcproPrefetch = navContent.includes('prefetch={false}') && 
                                      (navContent.includes('GCPro') || navContent.includes('GC Pro'));
    ...
    navGrid.grid.hasManualBorders = navContent.includes('border-') && 
                                   !navContent.includes('border-transparent');

    navGrid.grid.usesContainers = navContent.includes('container') || 
                                 navContent.includes('Container');
  } else {
